...
export type LyrikProgress = { version:1; completed:LyrikPhaseId[]; completedIntroductionChapters:number[]; completedKnowledgeChapters:number[]; completedTogetherChapters:number[]; completedPracticeChapters:number[]; completedChallengeChapters:number[]; completedTransferChapters:number[]; completedChapters:number[]; level:LyrikLevel; strengths:Record<string,number>; attempts:number };
const empty: LyrikProgress = { version:1,completed:[],completedIntroductionChapters:[],completedKnowledgeChapters:[],completedTogetherChapters:[],completedPracticeChapters:[],completedChallengeChapters:[],completedTransferChapters:[],completedChapters:[],level:1,strengths:{},attempts:0 };
const eventName = "lyrik-progress-change";
let cachedRaw:string|null|undefined;
let cachedProgress:LyrikProgress=empty;
export function parseProgress(raw:string|null):LyrikProgress { try { const item=JSON.parse(raw ?? "null") as Partial<LyrikProgress>; if(item?.version!==1)return empty; const valid=(value:unknown)=>Array.isArray(value)?[...new Set(value.filter(chapter=>Number.isInteger(chapter)&&chapter>=1&&chapter<=9))] as number[]:[];return {version:1,completed:Array.isArray(item.completed)?item.completed:[],completedIntroductionChapters:valid(item.completedIntroductionChapters),completedKnowledgeChapters:valid(item.completedKnowledgeChapters),completedTogetherChapters:valid(item.completedTogetherChapters),completedPracticeChapters:valid(item.completedPracticeChapters),completedChallengeChapters:valid(item.completedChallengeChapters),completedTransferChapters:valid(item.completedTransferChapters),completedChapters:valid(item.completedChapters),level:[1,2,3].includes(item.level as number)?item.level!:1,strengths:item.strengths??{},attempts:Number.isInteger(item.attempts)?item.attempts!:0}; } catch { return empty; } }
...
export function completeOrientationPhase(chapter:number,phase:"introduction"|"knowledge"){const old=readProgress(),key=phase==="introduction"?"completedIntroductionChapters":"completedKnowledgeChapters";if(old[key].includes(chapter))return;write({...old,[key]:[...old[key],chapter]});}
