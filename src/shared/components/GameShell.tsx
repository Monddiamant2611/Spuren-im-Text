...
export function GameShell() {
  ...
  const completeChapter01 = (session: Chapter01Session) => {
    ...
    for (const key of ["text_structure", "stage_direction", "speaker_assignment", "scene_orientation", "information_state"]) {
      const events = session.competencyEvents.filter((item) => item.competency === key);
      competencies[key] = { value: events.filter((item) => item.success).length + 2, level: "developing" };
    }
  ...
  const completeChapter02 = (session: Chapter02Session) => {
    const competencies = { ...state.competencies };
    for (const key of ["character_relationships", "direct_characterization", "indirect_characterization", "motivation", "conflict_analysis", "character_development", "knowledge_state_analysis", "evidence_reasoning"]) {
      const events = session.competencyEvents.filter((item) => item.competency === key);
      competencies[key] = { value: events.filter((item) => item.success).length + 2, level: "developing" };
    }
  ...
  const completeChapter03 = (session: Chapter03Session) => {
    const competencies = { ...state.competencies };
    for (const key of ["situation_analysis", "context_analysis", "cause_effect", "causal_reasoning", "knowledge_state_analysis", "evidence_reasoning", "relevance_selection", "unsupported_claim_detection"]) {
      const events = session.competencyEvents.filter((item) => item.competency === key);
      competencies[key] = { value: events.filter((item) => item.success).length + 2, level: "developing" };
    }
  ...
  const completeChapter04 = (session: Chapter04Session) => {
    const competencies = { ...state.competencies };
    for (const key of ["dialogue_analysis","conversation_goals","speech_acts","conversation_development","conflict_analysis","language_analysis","nonverbal_analysis","staging_reasoning","evidence_reasoning","perspective_analysis"]) {
      const events = session.competencyEvents.filter((item) => item.competency === key); competencies[key] = { value: events.filter((item) => item.success).length + 2, level: "developing" };
    }
  ...
  const completeChapter05 = (session: Chapter05Session) => {
    const competencies = { ...state.competencies };
    for (const key of ["relevance_selection","interpretation","evidence_reasoning","hypothesis_testing","claim_validation","argument_structure","critical_revision","staging_reasoning"]) { const events=session.competencyEvents.filter(item=>item.competency===key);competencies[key]={value:events.filter(item=>item.success).length+2,level:"developing"}; }
  ...
  const startFinale = () => {
    ...
    persist({ ...state, ...snapshot, currentChapter: "finale", finaleStarted: true, performanceState: "PERFORMANCE_RUNNING", progress: { ...state.progress, finale_ready: true } });
  };
  ...
  if (screen === "finale") return <Finale state={state} onStart={startFinale} onComplete={completeFinale} onExit={() => setScreen("start")} />;
