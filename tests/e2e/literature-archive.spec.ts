...
for (const viewport of viewports) test(`archive flow fits ${viewport.width}x${viewport.height}`, async ({ page }) => {
  ...
  const open=page.getByRole("button", { name: /Archiv öffnen/ });await open.click();if(await page.getByRole("button", { name: "Ambiguität" }).count()===0)await open.click();await expect(page.getByRole("button", { name: "Ambiguität" })).toBeVisible();
