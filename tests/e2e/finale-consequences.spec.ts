...
async function seed(page:import("@playwright/test").Page,value:ReturnType<typeof state>){await page.goto("/");await page.evaluate(({key,value})=>localStorage.setItem(key,JSON.stringify(value)),{key,value});await page.reload();await page.getByRole("button",{name:"Fortsetzen"}).click()}
