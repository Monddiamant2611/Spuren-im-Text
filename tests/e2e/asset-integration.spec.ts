...
for(const viewport of [{width:1920,height:1080},{width:1366,height:768},{width:1180,height:820},{width:1024,height:768},{width:390,height:844}])test(`production stage assets remain contained at ${viewport.width}x${viewport.height}`,async({page})=>{await page.setViewportSize(viewport);await save(page,"chapter_04",{chapter_04:{round:8,seenGlossaryIntroductions:["chapter_04"]}},["chapter_01","chapter_02","chapter_03"]);await page.waitForFunction(()=>[...document.querySelectorAll<HTMLImageElement>(".causal-stage img")].every(image=>image.complete&&image.naturalWidth>0));const result=await page.locator(".causal-stage").evaluate(stage=>{const box=stage.getBoundingClientRect();const images=[...stage.querySelectorAll("img")];return{overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth,loaded:images.every(image=>image.naturalWidth>0),contained:images.filter(image=>image.classList.contains("causal-character")).every(image=>{const rect=image.getBoundingClientRect();return rect.left>=box.left-1&&rect.right<=box.right+1&&rect.top>=box.top-1&&rect.bottom<=box.bottom+1})}});expect(result).toEqual({overflow:0,loaded:true,contained:true})});
