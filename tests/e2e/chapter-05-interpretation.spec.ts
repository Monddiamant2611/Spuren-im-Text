...
test("chapter 5 builds and persists a complete evidence-based interpretation without staging",async({page})=>{
 ...
 for(const item of classificationCards){await page.getByRole("button",{name:item.text}).click();await page.getByRole("button",{name:({observation:"Textbeobachtung",analysis:"Analyse",interpretation:"Interpretation",unsupported:"nicht ausreichend belegt"} as Record<string,string>)[item.target],exact:true}).click()}
