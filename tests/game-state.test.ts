...
describe("game state", () => {
  ...
  it("repairs missing and malformed chapter-session fields", () => { const restored = hydrateSession({ round: 99, signalAnswers: "broken", certaintyAssignments: null }, initialChapter01Session, 7); expect(restored).toMatchObject({ round: 1, signalAnswers: [], certaintyAssignments: {} }); });
});
