...
import { completeEpikChapter, EPIK_PROGRESS_KEY, normalizeCompletedChapters, parseEpikProgress, readEpikProgress, resetEpikProgress } from "../src/games/epik/epik-progress";
...
describe("lokaler Epik-Lernfortschritt", () => {
  ...
  it("speichert keine Diagnose-, Antwort- oder Zufallsdaten", () => { completeEpikChapter(1); expect(JSON.parse(window.localStorage.getItem(EPIK_PROGRESS_KEY)!)).toEqual({ version: 1, completedChapters: [1] }); });
