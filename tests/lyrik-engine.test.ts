...
describe("Lyrikwerkstatt",()=>{
  ...
  it("liest alte und neue Orientierungsstände ohne falsche Haken",()=>{const old=parseProgress(JSON.stringify({version:1,completedTogetherChapters:[6]}));expect(old.completedIntroductionChapters).toEqual([]);expect(old.completedKnowledgeChapters).toEqual([]);const current=parseProgress(JSON.stringify({version:1,completedIntroductionChapters:[3,3,11],completedKnowledgeChapters:[3]}));expect(current.completedIntroductionChapters).toEqual([3]);expect(current.completedKnowledgeChapters).toEqual([3]);});
